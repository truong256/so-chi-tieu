...
import type { ParsedReceiptResult, ReceiptItem, TransactionType } from "@/app/finance-types";

export const maxDuration = 35; // Allow up to 35 seconds for image analysis

const RECEIPT_SYSTEM_PROMPT = `Bạn là hệ thống AI chuyên gia trích xuất dữ liệu từ hình ảnh hóa đơn, chứng từ thanh toán và phiếu thu chi (AI Receipt Parser) cho ứng dụng quản lý chi tiêu Sổ Chi Tiêu.
...
export async function POST(request: Request) {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      // Handle data URL prefix if present: "data:image/jpeg;base64,..."
    ...
    // Candidate Gemini Models for Multimodal
    ...
    // Sanitize numbers for Vietnamese currency
    ...
    // Sanitize dates (YYYY-MM-DD or DD/MM/YYYY or DD-MM-YYYY)
    ...
    if (typeof parsed.category === "string" && parsed.category.trim()) {
      ...
      const matched = categoriesList.find(c => c.toLowerCase() === rawCat || c.toLowerCase().includes(rawCat) || rawCat.includes(c.toLowerCase()));
      if (matched) category = matched;
    }

    // Check payment method & warnings
