  ...
  async fetch(request: Request, env: Env, ctx: ExecutionContext): Promise<Response> {
    ...
    if (url.pathname === "/api/chat" && request.method === "POST") {
      try {
        ...
        // Rule-based pre-filter — fast rejection for obvious off-topic
        ...
        // Build context text to inject into Gemini
        ...
        // Gemini API uses "contents" with roles; inject system as first user turn
        ...
        for (const modelName of candidateModels) {
          try {
            ...
            // If it's not a 404 (model not found), don't try other models (e.g. 401 invalid key, 429 rate limit)
            if (res.status !== 404) {
              geminiRes = res;
              break;
            }
        ...
        if (!reply) {
          if (candidate?.finishReason === "SAFETY") {
            return Response.json({ error: "Nội dung bị chặn bởi bộ lọc an toàn." }, { status: 400 });
          }
          return Response.json({ error: "AI trả về phản hồi rỗng. Vui lòng thử lại." }, { status: 502 });
        }
